...
from main import prob_cond_bin
...
def run_timing_analysis(number_variables: int, num_repetitions: int = 3) -> dict:
    """
    ...
    for num_interest in range(number_variables + 1):
        ...
        for num_cond in range(available_for_cond + 1):
            ...
            num_configs = min(
                3,
                len(list(itertools.combinations(all_vars, num_interest)))
                if num_interest > 0
                else 1,
            )
            ...
            if num_interest == 0:
                ...
                interest_combos = list(itertools.combinations(all_vars, num_interest))[
                    :num_configs
                ]
                interest_configs = [
                    generate_mask(combo, number_variables) for combo in interest_combos
                ]
            ...
            for maskI in interest_configs:
                ...
                if num_cond == 0:
                    ...
                    cond_combos = list(
                        itertools.combinations(available_vars, num_cond)
                    )[
                        : min(
                            3,
                            len(list(itertools.combinations(available_vars, num_cond))),
                        )
                    ]
                ...
                for maskC, valC in cond_configs:
                    ...
                    for _ in range(num_repetitions):
                        ...
                        result = prob_cond_bin(
                            distribution, number_variables, maskC, valC, maskI
                        )
...
def print_results_table(results: dict) -> None:
    ...
    print(
        ...
        + "║"
        + RESET
        + f" {BOLD + MAGENTA}{headers}{RESET}"
        + " " * (total_width - len(headers) - 3)
        + CYAN
        + "║"
        + RESET
    )
...
def save_results_to_csv(results: dict, number_variables: int) -> None:
    ...
    # Guardar siempre en el directorio del proyecto como "timing_results.csv"
    csv_file = Path(__file__).with_name("timing_results.csv")
